class Gun {
  ...
  shoot (x, y, a, socket) {
    ...
    if (this.timer <= 0) {
      const posX = x + Math.cos(a + Math.PI / 12) * this.scl / 1.8
      const posY = y + Math.sin(a + Math.PI / 12) * this.scl / 1.8
      const b = new Bullet(posX, posY, Math.cos(a) * 25, Math.sin(a) * 25, this.scl)
...
class Bullet {
  ...
  generateID () {
    const alphabet = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 1, 2, 3, 4, 5, 6, 7, 8, 9]
    let result = ''
    for (let i = 0; i < 10; i++) {
